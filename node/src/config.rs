...
impl Config {
    ...
    pub fn _from_read<R: std::io::Read>(buf: &mut R) -> Result<Self, Box<dyn Error>> {
    ...
    pub fn _read_from_path<P: AsRef<std::path::Path>>(path: P) -> Result<Self, Box<dyn Error>> {
        let mut file = std::fs::OpenOptions::new().read(true).open(path)?;
        Self::_from_read(&mut file)
    }

    /// For writing a default configuration file.
    pub fn _write_template<W: std::io::Write>(buf: &mut W) -> Result<(), Box<dyn Error>> {
    ...
    pub fn _create_template_from_path<P: AsRef<std::path::Path>>(path: P) -> Result<(), Box<dyn Error>> {
        let mut file = std::fs::OpenOptions::new().create(true).write(true).open(path)?;
        Self::_write_template(&mut file)
    }
}
