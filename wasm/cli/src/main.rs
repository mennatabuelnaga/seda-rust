...
// #[command(about = "For interacting with the SEDA protocol.", long_about =
// None)] struct Options {
//     #[command(subcommand)]
...
fn main() {
    ...
    cmd.set_bin_name("seda");
    cmd.build();

    let matches = cmd.get_matches_from(vec!["--help"]);
    ...
    //             http_fetch(&url).start().then(call_self("http_fetch_result",
    // vec![]));         }
    //         Commands::Hello => {
    //             println!("Hello World from inside wasm");
    //         }
    //         // TODO how to remove double near specification here?
    //         // cargo run -- -c near cli view near mc.mennat0.testnet
    // get_nodes "{\"offset\":\"0\",\"limit\":\"2\"}"         Commands::View
    // {             chain,
    //             contract_id,
    //             method_name,
    //             args,
    //         } => {
    //             chain_view(chain, contract_id, method_name,
    // args.into_bytes())                 .start()
    //                 .then(call_self("chain_view_test_success", vec![]));
    //         }
    //         // cargo run -- -c near cli call near mc.mennat0.testnet
    // register_node         // "{\"socket_address\":\"127.0.0.1:8080\"}"
    // "870000000000000000000"         Commands::Call {
    //             chain,
    //             contract_id,
    //             method_name,
    //             args,
    //             deposit,
    //         } => {
    //             chain_call(chain, contract_id, method_name,
    // args.into_bytes(), deposit)                 .start()
    ...
    //             get_node(node_id);
    //         }
    //         // cargo run -- -c near cli register-node 127.0.0.1:8080
    // 870000000000000000000         Commands::RegisterNode {
    //             socket_address,
    //             deposit,
    //         } => {
    //             register_node(socket_address, deposit);
    //         }
    //         // cargo run -- -c near cli update-node 16
    // "SetSocketAddress(127.0.0.1:8000)"         Commands::UpdateNode {
    // node_id, command } => {             update_node(node_id, command);
    //         }
    ...
    //         }
    //         Commands::Run => {
    //             // This command is only handled by the actual node, not by
    // the WASM             unreachable!();
    //         }
    //     }
    // }
}
