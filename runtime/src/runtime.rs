...
use crate::{adapters::Adapters, config::VmConfig, context::create_vm_context, imports::create_wasm_imports};

pub struct VmResult {}

pub fn start_runtime(config: VmConfig, adapters: Arc<Mutex<Adapters>>) -> Result<VmResult, String> {
    ...
    let vm_context = create_vm_context(adapters);
