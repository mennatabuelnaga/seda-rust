...
// use super::{HostAdapters, InMemory, MemoryAdapter, RunnableRuntime, Runtime,
// TestAdapters, VmConfig};
...
// serde_json::from_str(&db_result.unwrap()).unwrap();     assert!(matches!
// (result, PromiseStatus::Fulfilled(_)));
...
//     let expected_result =
// reqwest::get(fetch_url).await.unwrap().text().await.unwrap();
...
//                 program_name: "consensus".to_string(),
//                 start_func:
// Some("memory_adapter_test_success".to_string()),                 debug:
// true,             },
...
//     let read_value: Result<Option<Vec<u8>>, _> =
// memory_adapter_ref.get("u8");     let expected =
// 234u8.to_le_bytes().to_vec();     let expected_str = format!("{expected:?}");
