...
use crate::{helpers::get_env_var, Result};
...
pub trait CliCommands: Send + Sync {
    ...
    async fn get_nodes(limit: u64, offset: u64) -> Result<()> {
        ...
        let response =
            Self::view_seda_server("get_nodes", rpc_params![contract_id, limit, offset, near_server_url]).await?;
    ...
    async fn get_node_owner(node_id: u64) -> Result<()> {
        ...
        let response =
            Self::view_seda_server("get_node_owner", rpc_params![contract_id, node_id, near_server_url]).await?;
