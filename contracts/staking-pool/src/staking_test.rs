...
mod tests {
    ...
    fn test_deposit_withdraw() {
        ...
        // deposit by pretending to be the token contract
        emulator.update_context(token(), deposit_amount);
        emulator
            .contract
            .deposit(deposit_amount.into(), bob().try_into().unwrap());
    ...
    //     let expected_amount =
    //         deposit_amount + ntoy((yton(deposit_amount) * 90_000 +
    // n_locked_amount / 2) / n_locked_amount);     assert_eq_in_near!(
    //         emulator
    //             .contract
    //             .get_account_staked_balance(bob().try_into().unwrap())
    //             .0,
    ...
    //     emulator.update_context(bob(), 0);
    //     emulator.contract.ping();
    //     // previous balance plus (1_090_000 / 1_100_030)% of the 90_000
    // reward (rounding     // to nearest).
    //     assert_eq_in_near!(
    //         emulator
    //             .contract
    //             .get_account_staked_balance(bob().try_into().unwrap())
    //             .0,
    //         expected_amount + ntoy((yton(expected_amount) * 90_000 +
    // n_locked_amount / 2) / n_locked_amount)     );
    //     // owner earns 10% with the fee and also small percentage from
    // restaking.     assert_eq_in_near!(
    //         emulator
    //             .contract
    //             .get_account_staked_balance(owner().try_into().unwrap())
    //             .0,
    //         ntoy(10_000) + ntoy(10_000) + ntoy((10_000u128 * 90_000 +
    // n_locked_amount / 2) / n_locked_amount)     );
    ...
    //     assert_eq_in_near!(acc.staked_balance.0, deposit_amount / 2 +
    // ntoy(10));     assert!(!acc.can_withdraw);
    ...
    // /// Test that two can delegate and then undelegate their funds and
    // rewards /// at different time.
    // #[test]
