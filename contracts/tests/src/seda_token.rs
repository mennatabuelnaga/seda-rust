...
//     let transfer_amount = U128::from(parse_near!("100 N"));
//     let worker = workspaces::sandbox().await?;
//     let (contract, _, staking_pool_contract) =
// init(&worker,initial_balance).await?;

//     // defi contract must be registered as a FT account
//     register_user(&contract, staking_pool_contract.id()).await?;
...
//                 .args_json((staking_pool_contract.id(), transfer_amount,
// Option::<String>::None, "10"))                 .deposit(ONE_YOCTO)
...
// anyhow::Result<()> {     let initial_balance =
// U128::from(parse_near!("10000N"));     let transfer_amount =
// U128::from(parse_near!("100 N"));     let worker =
// workspaces::sandbox().await?;     let (contract, _, staking_pool_contract) =
// init(&worker, initial_balance).await?;
...
//         .args_json((staking_pool_contract.id(), transfer_amount,
// Option::<String>::None, "take-my-money"))         .max_gas()
//         .deposit(ONE_YOCTO)
//         .transact()
//         .await?;
//     assert!(res.is_success());
...
//     let transfer_amount = U128::from(parse_near!("100 N"));
//     let worker = workspaces::sandbox().await?;
//     let (contract, _, staking_pool_contract) = init(&worker,
...
// anyhow::Result<()> {     let initial_balance = U128::from(parse_near!("10000
// N"));     let refund_amount = U128::from(parse_near!("50 N"));
//     let transfer_amount = U128::from(parse_near!("100 N"));
//     let worker = workspaces::sandbox().await?;
//     let (contract, _, staking_pool_contract) = init(&worker,
// initial_balance).await?;
...
//         .await?
//         .json::<U128>()?;
//     assert_eq!(initial_balance.0 - transfer_amount.0 + refund_amount.0,
// root_balance.0);     assert_eq!(transfer_amount.0 - refund_amount.0,
// defi_balance.0);
...
// anyhow::Result<()> {     let initial_balance = U128::from(parse_near!("10000
// N"));     let transfer_amount = U128::from(parse_near!("100 N"));
//     let worker = workspaces::sandbox().await?;
//     let (contract, _, staking_pool_contract) = init(&worker,
// initial_balance).await?;
